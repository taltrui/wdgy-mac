...
def build_config():
  ...
  for index, filename in enumerate(os.listdir(CONFIG_PATH), start=1):
    ...
    if 'name' not in data:
      texthelper.print_error('El archivo ' + filename + ' no tiene una key "name", no fue agregado como opcion')
      continue

    if 'apps' not in data and 'commands' not in data:
      texthelper.print_error('El archivo ' + filename + ' no tiene apps ni comandos, no fue agregado como opcion')
      continue
    ...
    if 'apps' in data:
      ...
      texthelper.print_warning('El archivo ' + filename + ' no tiene ninguna app')

    if 'commands' in data:
      config_dict[index]['commands'] = data['commands']
    else: 
      texthelper.print_warning('El archivo ' + filename + ' no tiene ningun comando')

    texthelper.print_success('Archivo ' + filename + ' cargado satisfactoriamente')
    texthelper.new_line()
